...
import { RMA_FORM_FIELDS, DEFAULT_RMA_STATUS, blankRmaCase, rmaFieldFromRow } from '../config/rma';
...
// --- RMA CASES ---------------------------------------------------------------------------------
// Imports the RMA form's own template (RMA_FORM_FIELDS in config/rma.js) — one column per field,
// header text matching exactly what the exporter and blank template write, so exporting a sheet,
// editing it, and importing it back round-trips cleanly.

// Duplicate key = the case's own RMA number. A row with no number (a genuinely new case) always
// creates rather than matches.
const rmaDuplicateKey = (rmaNo) => String(rmaNo || '').trim().toUpperCase();

// Returns { created, updated, skipped, errors, serialsMatched, serialsUnmatched }.
export const importRmaCases = async (rows, { onDuplicate = 'skip', defaultTeamId = '' } = {}) => {
  const result = { created: 0, updated: 0, skipped: 0, errors: [], serialsMatched: 0, serialsUnmatched: 0 };
  const validTeams = storageService.getTeams();
  const existing = storageService.getRmaCases();
  const byKey = new Map(existing.filter((c) => c.rmaNo).map((c) => [rmaDuplicateKey(c.rmaNo), c]));
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    for (const field of RMA_FORM_FIELDS) {
      values[field.key] = rmaFieldFromRow(field, pickField(row, [normalizeHeader(field.header)]));
    }

    if (!values.customerName && values.serials.length === 0) {
      result.errors.push({ rowNumber, reason: 'Blank row — no customer name or serial number', raw: row });
      continue;
    }
    ...
    if (rmaCase.serials.length > 0) {
      const resolved = await storageService.resolveRmaFromSerial(rmaCase.serials[0]);
      if (resolved) {
        ...
        rmaCase.customerName = rmaCase.customerName || resolved.customerName;
